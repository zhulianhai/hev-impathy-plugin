# Makefile for hev-impathy-plugin
 
PP=cpp
CC=gcc
CCFLAGS=-g -fPIC `pkg-config --cflags telepathy-glib gtk+-2.0 nspr libxul`
LDFLAGS=-shared `pkg-config --libs telepathy-glib gtk+-2.0 nspr libxul`
 
SRCDIR=src
BINDIR=bin
BUILDDIR=build
 
TARGET=$(BINDIR)/libhevimpathyplugin.so
